...
$(document).ready(function () {
  ...
  $(".add-new").click(function () {
    ...
    var row = '<tr>' +
      '<td><input type="text" class="form-control" name="teacher_ID" id="teacher_ID"></td>' +
  ...
  $(document).on("click", ".delete", function () {
    ...
    if (keepData != ""){
      ...
      $.ajax({
        ...
        url: "http://localhost:8000/main/user/deleteTeacher",
...
function getTeacher() {
  $.ajax({
    type: "POST",
    contentType: "application/json",
    url: "http://localhost:8000/main/user/getTeacher",
    dataType: "json",
    success: function (customer) {
      ...
      if (customer != "") {
        ...
        for (var i = 0; i < customer.length; i++) {
          var row = '<tr>' +
            ...
            '<a class="delete" title="" data-toggle="tooltip" onclick="delTeacher(this)"><i class="material-icons">&#xE872;</i></a>' +
            '</td>' +
            '</tr>';
          $("table").append(row);
        }
...
function updateTeacher() {
  ...
  $.ajax({
    type: "POST",
    contentType: "application/json",
    url: "http://localhost:8000/main/user/updateTeacher",
